...
1. After that, locate the `src/main/java/chatbot.Shade.java` file, right-click it, and choose `Run chatbot.Shade.main()` (if the code editor is showing compile errors, try restarting the IDE). If the setup is correct, you should see something like the below as the output:
   ```
   Hello from
    ____        _        
   |  _ \ _   _| | _____ 
